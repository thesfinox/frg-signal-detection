...
def main(a: argparse.Namespace) -> int | str:
    ...
    ax.plot(snr, dimu2, "rx", label=r"$\text{dim}(u_{2})$", alpha=0.85)
    ax.plot(snr, dimu4, "gx", label=r"$\text{dim}(u_{4})$", alpha=0.85)
    ax.plot(snr, dimu6, "bx", label=r"$\text{dim}(u_{6})$", alpha=0.85)
    ax.set(
        xlabel=r"signal-to-noise ratio ($\beta$)", ylabel="canonical dimension"
    )
    ax.ticklabel_format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        default="mp_canonical_dimensions_snr.png",
        help="Output file",
    )
    parser.add_argument(
