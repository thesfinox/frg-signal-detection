...
__description__ = "Plot the phase space of the FRG flow: for each initial condition in the UV, we plot whether the trajectory ended in a symmetryc phase (u2 > 0) or not."
...
def main(a: argparse.Namespace) -> int | str:
    ...
    for d in a.data:
        ...
        coords.append([data["u2"][0], data["u4"][0], data["u6"][0]])
        phase.append(True if data["u2"][idx] > 0.0 else False)
        colors.append("g" if data["u2"][idx] > 0.0 else "r")
        labels.append("symmetric" if data["u2"][idx] > 0.0 else "broken")
