...
def main(argv: list[str] | None = None) -> int | str:
    ...
    parser.add_argument(
        "output", type=str, help="Output directory of the plots"
    )
    parser.add_argument(
        "--suffix", type=str, default=None, help="Suffix for the output files"
    )
    parser.add_argument(
        "--analytic",
        action="store_true",
    ...
    if not inp_dir_or_file.exists():
        logger.error(f"{inp_dir_or_file} does not exist!")
    ...
    if inp_dir_or_file.is_file():
        ...
        with open(inp_dir_or_file) as f:
